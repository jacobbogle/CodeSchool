...
window.addEventListener('load', bookBoxShape());
...
function bookBoxShape() {
    const images = document.querySelectorAll(".bookImage")
    images.forEach(image => {
        const randomHeight = Math.floor(Math.random() * 120) + 300
        const randomWidth = Math.floor(Math.random() * 120) + 200
...
function bookContentSizing() {
    ...
    window.addEventListener('mouseup', function(event) {
        ...
        const newWidth =  bookWidth * .95 + 'px';
        const newHeight = bookHeight * .94 + 'px';
